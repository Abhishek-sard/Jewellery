...
import { getOneMainBanner, getOneSecondBanner, updateMainBanner, updateSecondBanner } from "../../../Redux/Slice/bannerSlice";
import { PiListBulletsBold } from "react-icons/pi";

const EditSecondBanner = () => {
  const { id } = useParams();
  ...
  const { secondBanner, isLoading } = useSelector((state) => state.banner);

  // Fetch banner details
  useEffect(() => {
    dispatch(getOneSecondBanner(id)).then((result) => {
      console.log(result)
      if (result.payload) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await dispatch(updateSecondBanner({ formData,id }));
      console.log(result)
  ...
  return (
    <>
      <div className="cardHead -z-[0]">
        <h2 className="text-3xl font-semibold text-[#34395E]">Edit second Banner</h2>
      </div>
...
export default EditSecondBanner;
