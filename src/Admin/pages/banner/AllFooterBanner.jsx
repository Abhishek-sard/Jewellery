...
import { getAllBanners, getAllFooterBanners } from "../../../Redux/Slice/bannerSlice";
import api from "../../../axios";

const AllFooterBanner = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { footerBanners, isLoading } = useSelector((state) => state.banner);

  const [limit, setLimit] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    dispatch(getAllFooterBanners());
  }, [dispatch]);

  const total = footerBanners.length;
  const totalPages = Math.ceil(total / limit);
  const startIndex = (currentPage - 1) * limit;
  const showResult = footerBanners.slice(startIndex, startIndex + limit);
  ...
  const handleSearch = (e) => {
    ...
    const filtered = footerBanners.filter((banner) =>
  ...
  const handleEditBanner = (id) => {
    navigate("/admin/banner/footer-banner/edit/"+id);
  };

  const handleDeleteBanner = async (id) => {
    const res = await api.delete(`/api/inventory/deletefooterbanner/${id}`);
    if (res.data.success) {
  ...
  return (
    <>
      <div className="cardHead flex flex-col md:flex-row justify-between gap-4">
        <h2 className="text-3xl font-semibold text-[#34395E]">Footer Banners</h2>
        <Link
          to="/admin/banner/footer-banner/create"
...
export default AllFooterBanner;
