...
import { getOneFooterBanner, updateFooterBanner, } from "../../../Redux/Slice/bannerSlice";
import { PiListBulletsBold } from "react-icons/pi";

const EditMainFooter = () => {
  const { id } = useParams();
  ...
  const { footerBanner, isLoading } = useSelector((state) => state.banner);

  // Fetch banner details
  useEffect(() => {
    dispatch(getOneFooterBanner(id)).then((result) => {
      console.log(result)
      if (result.payload) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await dispatch(updateFooterBanner({ formData,id }));
      console.log(result)
      if (result?.payload?.success) {
        toast.success("Banner updated successfully!");
        navigate("/admin/banner/footer-banner");
      } else {
  ...
  return (
    <>
      <div className="cardHead -z-[0]">
        <h2 className="text-3xl font-semibold text-[#34395E]">Edit Footer Banner</h2>
      </div>

      <div className="mx-8 px-4 py-4 my-4 shadow-md">
        <Link
          to="/admin/banner/footer-banner"
        ...
        >
          <PiListBulletsBold /> All footer Banners
        </Link>
